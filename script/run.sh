...
pgrep -af github_action.jar | awk '{print $1}' | xargs -r kill -9;

echo "Starting the new Application..."
nohup java -jar github_action.jar --spring.profiles.active=local-docker > api.log &

echo "Successfully start Application!"
pgrep -af github_action.jar